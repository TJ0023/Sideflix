...
function Row({title, movies}:Props) {
  ...
  return (
    <div className='scrollbar-hide h-40 space-y-0.5 md:space-6-2' >
        <h2 
        className="w-56 cursor-pointer text-shadow- text-2xl font-semibold text-[#e5e5e5] 
        transition duration-200 hover:text-white md:text-4xl ">
            {title}
        </h2>
        ...
        <div ref={rowRef} className='flex scrollbar-hide items-center space-x-1 overflow-x-scroll md:space-x-2.5 md:p-2 overflow-y-hidden'>
            {movies.map((movie) => (
